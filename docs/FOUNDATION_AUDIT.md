...
|---|---|---|---|---|---|---|---|---|---|
| INST-01 | P0 | installer | `test-qemu-install` ends on `install backup gpt` and never prints `install auto` | `install_selftest` runs before `install_auto` and copies `BOOT/KERNEL.ELF` through ATA while interrupts are off. Each 2 KiB read walks the path and each ATA DMA allocated a fresh PRDT. The line is printed at the end of `write_gpt_backup`, before `write_esp` | installer owns the RAM selftest image; ATA driver owns the DMA bounce | BSP, interrupts off until after `lang_make_cc` | `test-qemu-install` | fix in tree, gate not re-run | copy of a very large `GAMES` tree can still exceed the gate; timeout was not raised |
| ATA-DMA-01 | P1 | ATA | install read does not finish | `ata_dma_xfer` called `pmm_alloc_contig` per transfer | device-local bounce, same single-owner rule as the IDE registers | one transfer at a time; the port is not re-entrant | install gate | fix in tree, gate not re-run | bounce is never freed; a second concurrent caller would alias it |
...
| TLB-IPI-01 | P1 | MM | a CPU that stops calling `mm_tlb_poll` trips `tlb shootdown timeout`, and a frame can be reused while a remote TLB entry remains | shootdown only bumped `mm_tlb_gen` and spun. `irq_dispatch` returned immediately for vector >= 48, and the LAPIC was left software-disabled | MM lock holder waits; each CPU acks its own `mm_tlb_seen` slot | IPI vector 0xF0 plus the old poll | SMP QEMU boot | fix in tree, QEMU not re-run | enabling the LAPIC must not steal the i8259 timer or ATA; that is the gate |
...
`foundation-gates`, the installed-desktop boot, Mine, Doom, self-host level 4, and a clean SMP run after the LAPIC enable are not results yet.
