...
| TGSI / VirGL shader backend | QEMU_TESTED for the proof shaders. Not STABLE | The triangle, depth, cube, textured cube, RGB varying, Lambert lighting, and the combined textured lit rotating mesh use TGSI from the compiler. The same boot prints `PASS: glsl compile`, `PASS: virgl varying`, `PASS: virgl lighting`, `PASS: virgl shader switch`, `PASS: virgl lit mesh`, and `PASS: shader mine link`. | 
