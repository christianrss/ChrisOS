...
the level-0 fixture and fourteen `kernel/metal` files, including `port.c`
and `heap.c`, then still links `serial.c` with `klog.c` and stubs. The
gate is `docs/KCC_STATUS.md`. It does not flip SH4.
