#include "aes.h"

static const uint8_t sbox[256] = {
    
};

static const uint8_t rcon[11] = {
    0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36
};

static uint8_t xt(uint8_t a) {
    return (uint8_t)((a << 1) ^ ((a & 0x80u) ? 0x1bu : 0));
}

static void expand(const uint8_t key[16], uint8_t rk[176]) {
    int i;
    for (i = 0; i < 16; ++i) {
        rk[i] = key[i];
    }
    for (i = 4; i < 44; ++i) {
        uint8_t t[4];
        int j;
        t[0] = rk[(i - 1) * 4];
        t[1] = rk[(i - 1) * 4 + 1];
        t[2] = rk[(i - 1) * 4 + 2];
        t[3] = rk[(i - 1) * 4 + 3];
        if ((i % 4) == 0) {
            uint8_t x = t[0];
            t[0] = (uint8_t)(sbox[t[1]] ^ rcon[i / 4]);
            t[1] = sbox[t[2]];
            t[2] = sbox[t[3]];
            t[3] = sbox[x];
        }
        for (j = 0; j < 4; ++j) {
            rk[i * 4 + j] = (uint8_t)(rk[(i - 4) * 4 + j] ^ t[j]);
        }
    }
}

void aes128_encrypt(const uint8_t key[16], const uint8_t in[16], uint8_t out[16]) {
    uint8_t rk[176];
    uint8_t s[16];
    int round;
    int i;
    expand(key, rk);
    for (i = 0; i < 16; ++i) {
        s[i] = (uint8_t)(in[i] ^ rk[i]);
    }
    for (round = 1; round <= 10; ++round) {
        uint8_t t[16];
        for (i = 0; i < 16; ++i) {
            t[i] = sbox[s[i]];
        }
        {
            uint8_t a;
            a = t[1]; t[1] = t[5]; t[5] = t[9]; t[9] = t[13]; t[13] = a;
            a = t[2]; t[2] = t[10]; t[10] = a;
            a = t[6]; t[6] = t[14]; t[14] = a;
            a = t[15]; t[15] = t[11]; t[11] = t[7]; t[7] = t[3]; t[3] = a;
        }
        if (round != 10) {
            for (i = 0; i < 16; i += 4) {
                uint8_t a = t[i], b = t[i + 1], c = t[i + 2], d = t[i + 3];
                uint8_t a2 = xt(a), b2 = xt(b), c2 = xt(c), d2 = xt(d);
                s[i] = (uint8_t)(a2 ^ b2 ^ b ^ c ^ d);
                s[i + 1] = (uint8_t)(a ^ b2 ^ c2 ^ c ^ d);
                s[i + 2] = (uint8_t)(a ^ b ^ c2 ^ d2 ^ d);
                s[i + 3] = (uint8_t)(a2 ^ a ^ b ^ c ^ d2);
            }
        } else {
            for (i = 0; i < 16; ++i) {
                s[i] = t[i];
            }
        }
        for (i = 0; i < 16; ++i) {
            s[i] ^= rk[round * 16 + i];
        }
    }
    for (i = 0; i < 16; ++i) {
        out[i] = s[i];
    }
}
