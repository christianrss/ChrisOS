...
static int run_qh(uint32_t first_td) {
    ...
    return td_wait(first_td - page_phys());
}
            ...
            for (fn = 0; fn < 8; ++fn) {
                ...
                for (port = 0; port < 2; ++port) {
                    ...
                    if (!port_reset(g_usb.io, port))
                        continue;
                    g_usb.addr = 0;
                    if (control(0, 0x00, 5, 1, 0, 0, 0, 0) != 0) {
                        serial_puts("usb addr\n");
                        continue;
                    }
                    g_usb.addr = 1;
                    if (control(1, 0x80, 6, 0x0200, 0, cfg, 9, 1) != 0) {
                    ...
                    if (ncfg > 128)
                        ncfg = 128;
                    if (control(1, 0x80, 6, 0x0200, 0, cfg, ncfg, 1) != 0) {
                        serial_puts("usb cfg\n");
                        continue;
                    }
                    g_usb.ep_in = 0;
                    ...
                    if (!g_usb.ep_in || !g_usb.ep_out) {
                        serial_puts("usb ep\n");
                        continue;
                    }
                    if (control(1, 0x00, 9, 1, 0, 0, 0, 0) != 0) {
                        serial_puts("usb set\n");
                        continue;
                    }
                    g_usb.tog_in = 0;
                    ...
                    g_usb.sectors = ((uint32_t)cap[0] << 24) | ((uint32_t)cap[1] << 16) |
