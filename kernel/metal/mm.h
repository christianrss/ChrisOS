...
/* Block until every online CPU has invalidated the latest unmap.
 * Remote CPUs are poked with a LAPIC IPI (vector 0xF0) and invlpg that
 * range. A CR3 reload here killed the machine mid-instruction. mm_tlb_poll
 * remains the ack path for a CPU that has not enabled its LAPIC yet. */
