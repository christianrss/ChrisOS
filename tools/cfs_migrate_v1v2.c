...
#define V1_INODE_LBA   5u
#define V1_DATA_LBA    21u
#define V1_INODE_SIZE  64u
#define V1_DIRECT      12u
#define V1_DIRENT      32u
#define V1_NAME_MAX    24u
...
static int v1_inode(uint32_t id, uint8_t out[64]) {
    uint8_t sec[512];
    uint32_t per = 512u / V1_INODE_SIZE;
    if (src_read(V1_INODE_LBA + id / per, sec) != 0) return -1;
    memcpy(out, sec + (id % per) * V1_INODE_SIZE, 64);
    return 0;
}
...
static int copy_file(Cfs *dst, const char *path, uint32_t ino) {
    ...
    uint16_t type;
    if (v1_inode(ino, raw) != 0) return -1;
    type = u16(raw + 0);
    size = u32(raw + 4);
    if (type != 1) return 0;
    if (size > 6144u) size = 6144u;
    memset(data, 0, sizeof(data));
    for (b = 0; done < size && b < V1_DIRECT; b++) {
        uint8_t sec[512];
        lba = u32(raw + 12 + b * 4);
        if (!lba) break;
...
static int walk(Cfs *dst, uint32_t dir_ino, const char *prefix) {
    uint8_t raw[64];
    uint32_t b, slot;
    if (v1_inode(dir_ino, raw) != 0) return -1;
    if (u16(raw + 0) != 2) return 0;
    for (b = 0; b < V1_DIRECT; b++) {
        uint32_t lba = u32(raw + 12 + b * 4);
        uint8_t sec[512];
        if (!lba) continue;
        if (src_read(lba, sec) != 0) return -1;
        for (slot = 0; slot < 16u; slot++) {
            uint8_t *e = sec + slot * V1_DIRENT;
            uint32_t ino = u32(e);
            uint8_t nlen = e[5];
            uint8_t type = e[4];
            char name[25];
            char path[96];
            uint32_t i;
            if (!ino || !nlen || nlen > V1_NAME_MAX) continue;
            for (i = 0; i < nlen; i++) name[i] = (char)e[8 + i];
            name[nlen] = 0;
            path[0] = 0;
            if (prefix[0]) {
                snprintf(path, 96, "%s/%s", prefix, name);
            } else {
                snprintf(path, 96, "%s", name);
            }
            if (type == 2) {
                int mrc = cfs_mkdir(dst, path);
...
int main(int argc, char **argv) {
    ...
    uint32_t i;
    if (argc < 3) {
        fprintf(stderr, "usage: cfs_migrate_v1v2 src.img dst.img\n");
        return 1;
    }
    ...
    if (u32(super) != 0x31534643u || u16(super + 4) != 1u) {
        fprintf(stderr, "src is not CFS v1\n");
        return 4;
    }
