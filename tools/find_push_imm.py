    ...
    if op == "PUSH" and p + 5 <= cs:
        ...
        if imm == target:
            ...
            for _ in range(6):
                ...
                extra = ""
                if o == "PUSH":
                    extra = "=%d" % struct.unpack_from("<i", code, q + 1)[0]
                parts.append("%d:%s%s" % (q, o, extra))
                q += nn
            print(" | ".join(parts))
    p += n

v = 27021597764222984
print("bad ptr", hex(v), "low32", hex(v & 0xFFFFFFFF), "high32", hex((v >> 32) & 0xFFFFFFFF))
